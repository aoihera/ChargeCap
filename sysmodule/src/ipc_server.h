 ...
 * --------------------------------------------------------------------------
 *
 * Vendored from sys-clk / Horizon-OC. The only change is that
 * ipcServerProcess() takes a timeout, which lets a single-threaded sysmodule
 * serve IPC and run a periodic tick without spawning a second thread (and
...
#define IPC_SERVER_MAX_DATA_SIZE (0x100 - 0x10 - sizeof(IpcServerRawHeader))
...
/* Waits up to `timeout` ns for either a new session or a request.
 * Returns KERNELRESULT(TimedOut) if nothing happened, which the caller uses
 * as its tick signal. */
